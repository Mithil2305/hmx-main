...
```
Browser (React + Vite)
  └── src/pages/          ← Page components (role-based)
  └── src/services/api.ts ← Axios client (JWT auto-attached)
  └── src/contexts/       ← AuthContext, SocketContext
        ↓  HTTP /api/*
Backend (Flask, port 5000)
  └── backend/app.py      ← All routes + DB + Socket.IO (8k lines)
  └── backend/config.py   ← PhonePe credentials
  └── backend/routes/     ← businessBooking.py, payment_routes.py
        ↓
SQLite (backend/hmx.db)
```

Vite proxies `/api` and `/socket.io` → `http://127.0.0.1:5000` in development (see `vite.config.ts`).
...
- **File:** `backend/hmx.db`
- **Connection:** Direct `sqlite3` in `backend/app.py` — no ORM
- **Settings:** 30s timeout, `PRAGMA journal_mode=WAL`, `PRAGMA foreign_keys=ON`
- **Init:** `init_db()` runs on app startup and creates tables if missing

---

### **Authentication Flow**

1. `POST /api/auth/login` → Flask returns a JWT (HS256)
2. Token stored in `localStorage` by `AuthContext`
3. Axios interceptor in `src/services/api.ts` auto-adds `Authorization: Bearer <token>` to every request
4. Backend validates with `@token_required` decorator, extracts `{ user_id, role }` from payload

**JWT payload structure:**
```json
{ "user_id": 1, "role": "pilot|editor|referral|admin|client", "iat": 1234567890 }
```
...
Client submits booking form
  → POST /api/bookings
  → Cost = Base Cost × Size Multiplier
  → POST /api/payment/initiate  →  PhonePe hosted page
  → PhonePe webhook → POST /api/payment/callback
  → booking.payment_status = 'completed'
  → Admin assigns pilot + editor
  → Pilot: claims → starts → completes
  → Earnings split applied
```

**Earnings split on completion:**
| Role     | Share |
|----------|-------|
| Pilot    | 65%   |
| Editor   | 10%   |
...
**Base costs by property type (INR):**
| Type | Cost |
|------|------|
| Restaurant / Retail Store | ₹12,000 |
| Office Space | ₹10,000 |
| Hotel | ₹18,000 |
| Real Estate / Shopping Mall | ₹20,000 |
| Adventure Park | ₹15,000 |

**Size multipliers:** Small 1×, Medium 1.5×, Large 2×, XL 3×, Enterprise = custom quote

---

### **Real-time Messaging (Socket.IO)**

- Each user joins room `user_{role}_{id}` on connect (JWT verified)
- Events: `message:new`, `message:sent`, `connection:ready`, `join:conversation`
- Frontend managed via `src/contexts/SocketContext.tsx`
...
| File | Purpose |
|------|---------|
| `backend/app.py` | Entire backend — all 60+ routes, DB schema, Socket.IO |
| `backend/config.py` | PhonePe production credentials |
| `backend/.env` | SMTP + PhonePe env vars (never commit) |
| `backend/routes/businessBooking.py` | Business booking cost logic |
| `backend/routes/payment_routes.py` | PhonePe payment endpoints |
| `src/services/api.ts` | All frontend API calls + Axios instance |
...
1. Add route in `backend/app.py` (or a new file under `backend/routes/`)
2. Protect with `@token_required` if auth is needed
3. Add a corresponding service method in `src/services/api.ts`
4. Role-gate the UI if needed — check `user.role` from `AuthContext`

---

### **Common Pitfalls**

- `backend/app.py` is a single 8k-line file — search carefully before adding duplicate routes
- SQLite WAL mode is set but the DB is a single file — avoid running multiple write-heavy processes simultaneously
- JWT secret is currently hardcoded in `app.py` — should be moved to `.env` before production
- PhonePe credentials in `config.py` are production keys — use sandbox credentials locally

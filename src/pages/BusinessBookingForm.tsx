...
const BusinessBookingForm: React.FC = () => {
	...
	const getTimeSlotOptions = () => {
			...
			"extra-large": [
				{
					...
					label: "Morning to Evening (7 AM - 7 PM)",
					duration: "12 hours",
				},
				{
					value: "late-morning-night",
					label: "Late Morning to Night (9 AM - 9 PM)",
					duration: "12 hours",
				},
				{
					value: "afternoon-night",
					label: "Afternoon to Night (11 AM - 11 PM)",
					duration: "12 hours",
				},
			],
			enterprise: [],
			...
			return (
				<div className="p-3 bg-yellow-50 border border-yellow-200 rounded-md">
					<p className="text-sm text-yellow-800">
						<strong>Custom Quote Required:</strong> For enterprise sizes
						(50,000+ sq. ft.), please contact us directly for a custom time slot
						arrangement.
					</p>
				</div>
			);
		...
		const slots =
			timeSlots[formData.businessSize as keyof typeof timeSlots] || [];
	...
	const calculateCost = () => {
		...
		const floorCount =
			formData.numFloors === "G"
				? 0
				: parseInt(formData.numFloors.split("+")[1]) || 0;
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (cost === 0) {
			...
			try {
				...
				alert(
					"Custom quote request submitted successfully! Our team will contact you within 24 hours.",
				);
				navigate("/dashboard");
			} catch (err: any) {
				setError(
					...
					<form onSubmit={handleSubmit} className="space-y-6">
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
							...
							<div className="relative">
								...
								{formData.phone && !/^[6-9]\d{9}$/.test(formData.phone) && (
									<p className="text-xs text-red-600 mt-1">
										Please enter a valid 10-digit mobile number starting with 6,
										7, 8, or 9
									</p>
								)}
							</div>
							...
							<div>
								...
								>
									...
									<option value="shopping-mall">
										Shopping Mall/Entertainment Zone
									</option>
									<option value="adventure-park">
										Adventure Park/Waterpark
									</option>
							...
							<div>
								...
								>
									...
									<option value="extra-large">
										Extra Large (10,000–50,000 sq. ft.)
									</option>
									<option value="enterprise">
										Enterprise (50,000+ sq. ft.)
									</option>
								</select>
							</div>
							...
							<div>
								...
								<p className="text-xs text-gray-500 mt-1">
									15% cost increase for each additional floor
								</p>
							</div>
							...
							<div className="md:col-span-2">
								...
								<p className="text-xs text-gray-500 mt-1">
									Enter exact location with landmark or search using Google Maps
								</p>
							</div>
							...
							<div className="md:col-span-2">
								...
								<p className="text-sm text-gray-500 mb-2">
									Select 3 consecutive dates (must be at least 5 days from
									today)
								</p>
								...
								{cost === 0 ? (
									<div>
										...
										<p className="text-sm text-gray-600 mt-2">
											For enterprise sizes (50,000+ sq. ft.), please contact us
											directly for a custom quote based on your specific
											requirements.
										</p>
									</div>
								) : (
						...
						<div className="flex justify-end space-x-4 pt-4">
							<button
								type="button"
								onClick={() =>
									isApproved ? setIsApproved(false) : navigate(-1)
								}
